...
- `papers/under-review/**`
- `projects/**` *except* (a) projects with `project_type: library_ingest` in their `Project_Brief.md` frontmatter and (b) anything under `projects/published/`
- `explorations/active/*/_pdfs/**`
- Any file whose own frontmatter declares `confidential_tier: local-only`
- Revision projects (folder names ending with `_revision` or `_revision_*`) are confidential again, regardless of their predecessor's status.
...
- `projects/published/**` — any project moved here after release on bioRxiv/medRxiv/arXiv or with a journal acceptance letter. Cloud LLMs (Claude Code, Codex CLI, Claude web via copy-paste) may read these folders. Promotion is gated by `scripts/publish_project.py` which requires a verified preprint URL or acceptance-letter PDF.
